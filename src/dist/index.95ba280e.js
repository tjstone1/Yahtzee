...
function handleSelect(e) {
    ...
    if (!target.innerText) {
        ...
        previewValues = [];
        return;
    } else if (!previewValues.includes(target)) return;
    else {
...
function clearPreviews(previewValues) {
    ...
    for (const value of previewValues)value.innerText = "";
    previewValues = [];
}
...
function checkScores(scores) {
    ...
    if (uniqueValues.length === 1) yahtzee = true;
    ...
    smallStraight = isConsecutive(scores.sort().slice(0, 4)) || isConsecutive(scores.sort().slice(1, 5)) ? 30 : 0;
